...
const seedDatabase = async () => {
    try {
        ...
        const seededSkills = await Skill.find().select("key title category level -_id");
        console.log("📚 Seeded Skills:");
        seededSkills.forEach(skill => {
            console.log("   " + skill.icon + " " + skill.title + " (" + skill.category + " - " + skill.level + ")");
        });
